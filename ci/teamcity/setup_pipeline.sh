...
PROJECT_ID="${TEAMCITY_PROJECT_ID:-Amy_SubscriptionService}"
PROJECT_NAME="${TEAMCITY_PROJECT_NAME:-subscription-service}"
BUILD_TYPE_ID="${TEAMCITY_BUILD_TYPE_ID:-Amy_SubscriptionService_Build}"
VCS_ROOT_ID="${TEAMCITY_VCS_ROOT_ID:-Amy_HttpsGithubComAmolsurjuseSubscriptionServiceRefsHeadsMain}"
GIT_URL="${SUBSCRIPTION_SERVICE_GIT_URL:-https://github.com/amolsurjuse/subscription-service}"
GIT_BRANCH="${SUBSCRIPTION_SERVICE_GIT_BRANCH:-main}"
DOCKER_IMAGE="${SUBSCRIPTION_SERVICE_DOCKER_IMAGE:-amolsurjuse/subscription-service}"
DEPLOY_VERSION_FILE="${SUBSCRIPTION_SERVICE_DEPLOY_VERSION_FILE:-charts/config/services/subscription-service/us/version/dev-version.yaml}"
...
set_parameters() {
  ...
  api PUT "/app/rest/buildTypes/id:${BUILD_TYPE_ID}/parameters/deployment.version.file" "$(jq -cn --arg value "${DEPLOY_VERSION_FILE}" '{name:"deployment.version.file",value:$value}')" >/dev/null
...
create_steps_if_empty() {
  ...
  local update_script
  read -r -d '' update_script <<'EOS' || true
set -eu
...
if [ ! -f "$FILE" ]; then
  echo "INFO: $FILE not found in k8s-platform, skipping version update"
  exit 0
fi
...
git commit -m "chore(subscription-service): deploy dev image tag ${BUILD}" || {
...
EOS
  ...
  api POST "/app/rest/buildTypes/id:${BUILD_TYPE_ID}/steps" "$(jq -cn --arg script "${update_script}" '{name:"Update build version",type:"simpleRunner",properties:{property:[{name:"script.content",value:$script},{name:"teamcity.step.mode",value:"default"},{name:"use.custom.script",value:"true"}]}}')" >/dev/null
